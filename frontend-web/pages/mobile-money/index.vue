...
const rules = {
  ...
  etablissement: [(v: any) => !!v || 'Opérateur obligatoire'],
}

// Les opérateurs sont gérés par l'administrateur : la liste vient du serveur.
...
async function charger() {
  ...
  try {
    const [txs, cpts, tauxData, etabs] = await Promise.all([
      api<Transaction[]>('/mobile-money/transactions'),
      api<Compte[]>('/comptes'),
...
async function soumettre() {
  ...
  try {
    ...
    if (form.sens === 'DECAISSEMENT' && form.noteFraisId) {
      ...
      tx = await api<Transaction>(`/mobile-money/notes/${form.noteFraisId}/payer`, {
        method: 'POST',
      ...
      tx = await api<Transaction>('/mobile-money/transactions', {
        method: 'POST',
        body: {
  ...
  <div>
    ...
    <div class="page-head">
      <div>
        ...
        <p class="page-sub">Opérations mobile money · <span class="taux-badge">{{ fmtTaux }}</span></p>
      </div>
      <div class="page-head-actions">
        <v-btn variant="outlined" color="primary" prepend-icon="mdi-refresh" rounded="lg" @click="charger">
          Actualiser
        </v-btn>
        <v-btn variant="outlined" color="primary" prepend-icon="mdi-notebook-outline" rounded="lg" to="/mobile-money/journal">
          Journal de mobile money
        </v-btn>
    ...
    <div v-if="etablissements.length" class="operateur-soldes">
      <div v-for="e in etablissements" :key="e.id" class="operateur-solde">
        <v-icon icon="mdi-cellphone" size="16" class="operateur-solde__icon" />
    ...
    <v-card class="classroom-card mt-4">
      ...
      <v-data-table
        ...
        :headers="[
          ...
          { title: 'Opérateur', key: 'etablissementNom', sortable: false },
      ...
      >
        ...
        <template #[`item.etablissementNom`]="{ item }">
          <v-chip v-if="item.etablissementNom" size="small" variant="tonal" color="deep-orange">
    ...
    <v-dialog v-model="dialogOuvert" max-width="500" persistent>
      <v-card class="dialog-card" rounded="xl">
        <div class="dialog-header dialog-header--mobile-money">
          <v-icon icon="mdi-cellphone" size="28" color="white" class="mb-2" />
          <div class="text-h6 font-weight-bold text-white">Nouvelle opération mobile money</div>
        ...
        <v-card-text class="pa-6 pt-5">
          ...
          <v-form ref="formRef">
            ...
            <!-- Opérateur télécom (obligatoire, encaissement et décaissement) -->
            <v-select
              v-model="form.etablissementId"
              :items="operateursOptions"
              label="Opérateur mobile money *"
              prepend-inner-icon="mdi-cellphone"
              no-data-text="Aucun opérateur configuré — voir Administration"
              variant="outlined"
              density="comfortable"
              rounded="lg"
...
.operateur-solde__icon { color: #ea580c; }
.operateur-solde__nom { font-size: 0.8rem; font-weight: 600; color: #7c2d12; }
...
.dialog-header--mobile-money {
  background: linear-gradient(140deg, #fb923c 0%, #ea580c 50%, #7c2d12 100%);
}
