...
/** Operation bancaire (encaissement/decaissement), miroir de {@link TransactionCaisse}. */
@Entity
@Table(name = "transactions_bancaires")
@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
...
public class TransactionBancaire {
    ...
    private String reference;   // TRB-2026-000045
    ...
    /** Banque par laquelle transite l'operation (porte son propre compte). */
    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "etablissement_id")
    @ToString.Exclude
    private EtablissementTresorerie etablissement;

    @OneToMany(mappedBy = "transactionBancaire", cascade = CascadeType.ALL, orphanRemoval = true)
    @Builder.Default
    @ToString.Exclude
    private List<EcritureGrandLivre> ecritures = new ArrayList<>();

    public void addEcriture(EcritureGrandLivre e) {
        e.setTransactionBancaire(this);
